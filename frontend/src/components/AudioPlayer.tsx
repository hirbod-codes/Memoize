...
export function AudioPlayer({ onClose }: { onClose?: React.MouseEventHandler<HTMLDivElement> }) {
    const { jsonAuthFetch } = useAuth()
    const { notify } = useNotification()
    const { setAudioIds, audioIds } = useContext(AudioContext)
    ...
    return (
        audioIds.length === 0
            ? <div className="p-3 flex flex-col size-full">
                <div className="grow flex flex-row items-start">
                    <div className="grow" />
                ...
                <div className="">
                    <img src="No_Music.png" alt="No Music" />
                </div>
                                    ...
                                    audioIds.map((v, i) =>
                                        <div key={i} className={i < audioIds.length ? 'w-full border-b border-outline' : 'w-full'}>
                                            <AudioCard audioId={v} clicked={() => { audio.jumpTo(i) }}>
                                                <Ripple>
                                                    <button onClick={() => { setAudioIds(audioIds.filter(f => f !== v)) }}>
