...
router.post('/', auth, async (req, res) => {
    const log = getLogger().child({ module: 'subscription', route: 'GET /api/subscription' });
...
const PAYMENT_CALLBACK_URL = '/api/subscription/verify'
router.get(`/verify/:subscriptionId/:authority`, unAuth, async (req, res) => {
    const log = getLogger().child({ module: 'subscription', route: `GET ${PAYMENT_CALLBACK_URL}/:subscriptionId/:authority` });
    ...
    try {
        ...
        // res.redirect(`https://${appUrl}/subscription/verify/check/${subscriptionId}`)
