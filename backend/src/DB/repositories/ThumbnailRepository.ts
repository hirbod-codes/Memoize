...
export class ThumbnailRepository implements IRepository, ISeedable, IDropable {
    ...
    async makePermanent(fileId: string) {
        try {
            return await ThumbnailRepository.filesCollection!.updateOne({ _id: ObjectId.createFromHexString(fileId) }, { $set: { 'metadata.temporary': false } }, { session: this.session })
        } catch (error) {
            console.log(error)
            return false
        }
    ...
    async makePermanentBulk(fileIds: string[]) {
        try {
            ...
            return await ThumbnailRepository.filesCollection!.bulkWrite(bulkWrites, { session: this.session })
        } catch (error) {
            console.log(error)
            return false
        }
    ...
    async deleteFileBulk(fileIds: string[]) {
        try {
            ...
            let r = await ThumbnailRepository.chunksCollection!.bulkWrite(chunksBulkWrites, { session: this.session })
            if (!r.ok)
                return false

            r = await ThumbnailRepository.filesCollection!.bulkWrite(filesBulkWrites, { session: this.session })
            if (!r.ok)
                return false
