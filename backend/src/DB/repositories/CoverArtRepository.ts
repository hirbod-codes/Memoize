...
export class CoverArtRepository implements IRepository, ISeedable, IDropable {
    ...
    private async getReadStream(fileId: string | ObjectId): Promise<GridFSBucketReadStream> {
        return CoverArtRepository.bucket!.openDownloadStream(typeof fileId === 'string' ? ObjectId.createFromHexString(fileId) : fileId)
    }

    private async getWriteStream(fileName: string, metadata: CoverArtMetadata): Promise<GridFSBucketWriteStream> {
        return CoverArtRepository.bucket!.openUploadStream(fileName, { metadata })
    }

    async upload(metadata: CoverArtMetadata, file: { fileName: string; bytes: Readable | Buffer | Uint8Array; contentType?: string }, temporary: boolean = true): Promise<string | false> {
        try {
    ...
    async getFileByAudioId(audioId: string): Promise<GridFSFile | undefined> {
        try {
            return (await CoverArtRepository.bucket!.find({ 'metadata.audioId': ObjectId.createFromHexString(audioId) }, { session: this.session }).toArray())[0];
        } catch (e) {
            console.error(e)
            return undefined
        }
    ...
    async getFilesByUserId(userId: string): Promise<GridFSFile | undefined> {
        try {
            return (await CoverArtRepository.bucket!.find({ 'metadata.userId': ObjectId.createFromHexString(userId) }, { session: this.session }).toArray())[0];
        } catch (e) {
            console.error(e)
            return undefined
        }
    ...
    async getFile(fileId: string): Promise<GridFSFile | undefined> {
        try {
            return (await CoverArtRepository.bucket!.find({ _id: ObjectId.createFromHexString(fileId) }, { session: this.session }).toArray())[0];
        } catch (e) {
            console.error(e)
            return undefined
        }
    ...
    async getFiles(fileIds: string[]): Promise<GridFSFile[]> {
        try {
            return await CoverArtRepository.bucket!.find({ _id: { $in: fileIds.map(id => ObjectId.createFromHexString(id)) } }, { session: this.session }).toArray();
        } catch (e) {
            console.error(e)
            return []
        }
    ...
    async deleteFilesByUserId(userId: string): Promise<boolean> {
        try {
            const cursor = CoverArtRepository.filesCollection!.find({ 'metadata.userId': userId }, { session: this.session })
    ...
    async deleteFileByAudioId(audioId: string): Promise<boolean> {
        try {
            const cursor = CoverArtRepository.filesCollection!.find({ 'metadata.audioId': ObjectId.createFromHexString(audioId) }, { session: this.session })
    ...
    async deleteFileForUserByAudioId(audioId: string, userId: string): Promise<boolean> {
        try {
            const cursor = CoverArtRepository.filesCollection!.find({ 'metadata.audioId': ObjectId.createFromHexString(audioId), 'metadata.userId': userId }, { session: this.session })
    ...
    async deleteFiles(fileIds: string[]): Promise<boolean> {
        try {
            let r = await CoverArtRepository.chunksCollection!.deleteMany({ files_id: { $in: fileIds.map(m => ObjectId.createFromHexString(m)) } }, { session: this.session })
            if (!r.acknowledged)
                return false

            r = await CoverArtRepository.filesCollection!.deleteMany({ _id: { $in: fileIds.map(m => ObjectId.createFromHexString(m)) } }, { session: this.session })
            if (!r.acknowledged)
                return false
    ...
    async deleteFile(fileId: string): Promise<boolean> {
        try {
            let r = await CoverArtRepository.chunksCollection!.deleteMany({ files_id: ObjectId.createFromHexString(fileId) }, { session: this.session })
            if (!r.acknowledged)
                return false
