...
export async function requestSmtp(email: string, locale: 'en' | 'fa', purpose?: SmtpPurpose): Promise<'sent' | 'cooldown' | 'failed'> {
    const log = getLogger().child({ module: 'smtp', email, purpose });
    ...
    if (await redis.exists(cooldownKey)) {
        log.debug('SMTP request blocked by cooldown');
        return 'cooldown';
    }
    ...
    await redis.set(cooldownKey, '1', 'EX', SMTP_REQUEST_COOLDOWN_SECONDS);
